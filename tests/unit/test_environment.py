...
# ---------- retired bundled_python field (spec 05 §3.9, post-D21 redesign) ----------


def test_bundled_python_field_in_archive_is_silently_ignored(tmp_path: Path) -> None:
    """v0.3.0 archives carried a `bundled_python` sub-object. The D21
    folder-bundle redesign retired the field; the bootstrap consumer must
    treat it as an unknown field per D9 (forward compatibility) and ignore
    it without raising.
    """
    env = {
        **valid_env(),
    ...
    pyz = make_pyz_with(tmp_path, env)
    # Loads cleanly; the field is dropped on the floor, not validated.
    loaded = load(pyz)
