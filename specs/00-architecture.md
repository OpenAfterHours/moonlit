...
**Bootstrap trust posture.** The bootstrap trusts `env.json` after schema validation per the D8 ordered checks. It validates the four `MOONLIT_*` env vars listed below per D16. The cache contents are treated as read-only after extraction. User code is invoked unsandboxed.
...
**Reserved env-var surface (D16).** The bootstrap reads exactly four environment variables: `MOONLIT_ROOT` (cache-root override), `MOONLIT_FORCE_EXTRACT` (force re-extract under lock), `MOONLIT_ENTRY_POINT` (override `env.entry_point`), `MOONLIT_DEBUG` (verbose tracing). "Truthy" is "present and non-empty after `os.environ.get(name, "")`" — the empty string is treated as unset; no special-casing of `0`/`false`. Any other `MOONLIT_*` name is reserved and unimplemented in v1; see `specs/03-bootstrap-runtime.md` for full semantics.
