...
`BuildConfig` (frozen dataclass): `project_root: Path`, `output_path: Path`, `entry_point: str | None`, `console_script: str | None`, `python_shebang: str`, `package: str | None`, `force: bool`, `verbosity: int`, `windows_exe: bool = False`, `python_version: str | None = None`, `bundle_python: bool = False`. Exactly one of `entry_point`/`console_script` is set (CLI enforces, exit 2 otherwise). When `bundle_python = True`, `output_path` is treated as a directory path (D21a); `windows_exe` is accepted alongside but is a no-op in that combination.
...
On success, exactly one of two shapes appears at `output_path`:

- **Default / `--windows-exe`** (`bundle_python = False`): exactly one new file at `output_path` — a PEP 441 zipapp prefixed with `b"#!" + python_shebang + b"\n"`, optionally preceded by a Windows launcher PE (D19).
- **Folder bundle** (`bundle_python = True`): exactly one new directory at `output_path` containing `<basename>.exe`, `<basename>.pyz`, and `_python/` (D21a). The `<basename>.pyz` byte-equals what a non-bundle build of the same flags would produce.
...
**Step 8.5 — Bundled-Python install** (only if `BuildConfig.bundle_python`; D21). `resolver.python_install(<tmp>/python, version=…)` runs `uv python install --install-dir <tmp>/python --no-bin --no-registry <X.Y>` and returns the single distribution dir uv produced under `<tmp>/python` (e.g. `cpython-3.13.7-windows-x86_64-none/`). `<X.Y>` is `BuildConfig.python_version` when set, else `f"{sys.version_info.major}.{sys.version_info.minor}"`. A regular file at `<dist_root>/python.exe` MUST exist; absence → `PythonBundleError` (13). This step MUST run AFTER step 8 — `build_id` is computed before any python-build-standalone bytes are read, so the staged-app cache is independent of which CPython patch uv shipped that day.

**Step 9 — Archive assembly.** Output shape dispatches on `BuildConfig`:

- `bundle_python = False`: emit a single artifact at `output_path`. The path begins with optional launcher PE bytes (when `windows_exe = True`, D19), then `b"#!" + python_shebang.encode("ascii") + b"\n"`, then the zip body.
- `bundle_python = True`: emit a directory at `output_path` whose contents are `<basename>.exe`, `<basename>.pyz`, and `_python/` (D21a).

In both modes the **zip body is the same**: site-packages → _bootstrap → __main__.py → env.json. Invariant I11 is restored to its pre-bundle simplicity: the zip body of a `--windows-exe` build and the zip body of a plain `.pyz` build of the same project + same other flags are byte-identical at the entry level. Invariant I11b is restated: in `--bundle-python` mode, `<basename>.pyz` byte-equals what a non-bundle build of the same flags would produce.

**9-single — Single-file write** (when `bundle_python = False`):
...
10. `os.replace(tmp_out, output_path)` (D15, atomic on POSIX and Windows). On POSIX, then `os.chmod(output_path, 0o755)` UNLESS `windows_exe` is set (D19d). On Windows, no-op regardless. On any failure between 9-single.2 and 9-single.10, `finally` unlinks `tmp_out` if it exists; `output_path` is never partially written.

**9-bundle — Folder-bundle assembly** (when `bundle_python = True`):

1. Pre-flight on `output_path` per D21g: parent must exist and be writable (else `OutputNotWritableError` 7); if `output_path` exists and is NOT a moonlit-recognized bundle directory → `OutputNotWritableError` (7); if it IS a moonlit-recognized bundle directory and `not force` → `OutputExistsError` (7). "Moonlit-recognized" means a directory containing `<basename>.exe`, `<basename>.pyz`, and `_python/python.exe` (where `<basename>` is `output_path.name`).
2. Resolve `<basename> = output_path.name`. Compute `staging_dir = output_path.with_name(f"{output_path.name}.tmp.{os.getpid()}")`. Create `staging_dir`.
3. Build `<basename>.pyz` inside `staging_dir` using the same body-write sequence as 9-single (steps 9-single.3–9, except the prefix step writes only the shebang — no launcher PE). The resulting `.pyz` is byte-identical to what a non-bundle build would produce given the same flags.
4. Copy the host-arch launcher bytes (D19a) verbatim to `staging_dir/<basename>.exe`. No appended zip, no shebang line — the launcher is a standalone PE.
5. Copy the step-8.5 distribution tree to `staging_dir/_python/`, preserving the directory structure under the dist root. Files are copied via `shutil.copyfile` for content; on POSIX hosts the source executable bits propagate via `shutil.copymode`. On Windows there are no per-file permission bits to preserve.
6. Atomically install `staging_dir` at `output_path` via the D4 directory-replace protocol. If a previous moonlit bundle was at `output_path` (the `--force` case), D4 renames it aside as `<output_path>.old.<pid>` and best-effort removes it after the swap. On any failure between 9-bundle.2 and 9-bundle.6, `finally` removes `staging_dir` if it exists; `output_path` is never partially written.
